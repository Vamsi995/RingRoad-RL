...
from Ring_Road.constants import RADIUS, LENGTH
...
class StateExtractor():
    ...
    def gap_front(self, veh):
        if veh.front_vehicle.central_angle - veh.central_angle < 0:
            sf = (2 * math.pi - veh.central_angle + veh.front_vehicle.central_angle) * RADIUS - LENGTH
        else:
            sf = (veh.front_vehicle.central_angle - veh.central_angle) * RADIUS - LENGTH
        return sf
    ...
    def _calculate_distances(self, agent):
        if agent.front_vehicle.central_angle - agent.central_angle < 0:
            ...
            sf = (agent.front_vehicle.central_angle - agent.central_angle) * RADIUS - LENGTH

        if agent.central_angle - agent.back_vehicle.central_angle < 0:
            sb = (2 * math.pi + agent.central_angle - agent.back_vehicle.central_angle) * RADIUS - LENGTH
        else:
            sb = (agent.central_angle - agent.back_vehicle.central_angle) * RADIUS - LENGTH
